...
// Martes 2026-06-02. Fortaleza UTC−3: 08:00–18:00 local = 11:00Z–21:00Z.
const FROM = new Date("2026-06-02T00:00:00Z");
const TO = new Date("2026-06-02T23:59:00Z");

describe("findFreeSlots", () => {
  it("día vacío, duración 60 → 10 slots cada hora desde 08:00 Fortaleza (11:00Z)", () => {
    ...
    expect(slots[slots.length - 1].toISOString()).toBe("2026-06-02T20:00:00.000Z"); // 17:00 local
  });
  ...
  it("el paso es la duración del servicio (90min)", () => {
    ...
    // 08:00,09:30,11:00,12:30,14:00,15:30 (termina 17:00) → 6; 17:00+90=18:30>18:00 no.
  ...
  it("no genera slots que se pasen del cierre (18:00)", () => {
    const slots = findFreeSlots({
      from: FROM,
      to: TO,
      durationMinutes: 60,
    ...
    const after18 = slots.filter((s) => s.toISOString() > "2026-06-02T20:00:00.000Z");
    expect(after18).toHaveLength(0);
  });
